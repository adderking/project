...
public class SqlUtils {
    ...
    public static SqlContext buildInsertSql(Object entity, NameHandler nameHandler) {
        ...
        try {
            beanInfo = ClassUtil.getSelfBeanInfo(clazz);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for (PropertyDescriptor pd : pds) {
            ...
            sql.append(pd.getName());//nameHandler.getColumnName(pd.getName()));
            args.append("?");
            params.add(value);
            sql.append(",");
            args.append(",");
    ...
    public static SqlContext buildUpdateSql(Object entity, NameHandler nameHandler) {
        ...
        for (PropertyDescriptor pd : pds) {
            ...
            String columnName = pd.getName();//nameHandler.getColumnName(pd.getName());
    ...
    public static SqlContext buildQueryCondition(Object entity, NameHandler nameHandler) {
        ...
        BeanInfo beanInfo = null;
        try {
            beanInfo = ClassUtil.getSelfBeanInfo(entity.getClass());
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for (PropertyDescriptor pd : pds) {
            ...
            condition.append(pd.getName());//nameHandler.getColumnName(pd.getName()));
            condition.append(" = ?");
            params.add(value);
            count++;
        }
